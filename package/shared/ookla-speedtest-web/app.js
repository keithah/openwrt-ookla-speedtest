...
function applyNetworkPreview(info){if(state.status!=='idle')return;state.isp=info.isp||state.isp;state.connection=(info.interface&&info.interface.name)||state.connection;state.network=info.network_context||state.network;if(info.server){if(state.server.id)state.server=info.server;else state.autoServer=info.server}render();notify()}
