...
function applySettings(settings){settings={default_mode:settings.default_mode||'router-internet',server_id:settings.server_id||'',history_retention:settings.history_retention||100,motion:settings.motion||'system',schedule_hours:settings.schedule_hours||0,terms_accepted:!!settings.terms_accepted};state.settings=settings;state.mode=settings.default_mode;state.server=settings.server_id?{id:String(settings.server_id),name:String(settings.server_id),sponsor:'',city:'',latency:'—'}:{name:'Auto',sponsor:'',city:'',latency:'—'};state.autoServer=null;return settings}
function saveSettings(changes){if(!initialized)return Promise.resolve(state.settings);return checked('save_settings',changes).then(function(settings){applySettings(settings);render();notify();loadNetworkPreview();return settings})}
function applyNetworkPreview(info){if(state.status==='done')return;state.isp=info.isp||state.isp;state.connection=info.connection_label||(info.interface&&info.interface.name)||state.connection;state.network=info.network_context||state.network;if(info.server){if(state.server.id)state.server=info.server;else state.autoServer=info.server}render();notify()}
function loadNetworkPreview(){return checked('network_info',{}).then(applyNetworkPreview).catch(function(){})}
...
function applyLocalSample(phase,value){state.status='running';state.phase=phase;state.gaugeValue=value;state.gaugeUnit=phase==='ping'?'ms':'Mbps';if(phase==='ping')state.ping=value;else{state[phase]=value;state.traces[phase]=state.traces[phase].concat([value]).slice(-120)}render();notify()}
...
function applyLiveStatus(payload,jobId){
  ...
  if(payload.result){state.isp=payload.result.isp||state.isp;state.connection=payload.result.connection_label||(payload.result.interface&&payload.result.interface.name)||state.connection;state.network=payload.result.network_context||state.network;if(payload.result.server)state.server=payload.result.server;if(payload.result.packetLoss!=null)state.loss=Number(payload.result.packetLoss)}
...
function shareResult(url,button){
  if(!url)return;
  var nav=window.navigator;
  if(nav&&typeof nav.share==='function'){nav.share({title:'Speedtest result',url:url}).catch(function(){});return}
  if(nav&&nav.clipboard&&typeof nav.clipboard.writeText==='function'){
    nav.clipboard.writeText(url).then(function(){
      if(!button)return;var original=button.textContent;text(button,'Copied!');setTimeout(function(){text(button,original)},1500);
    }).catch(function(){});
    return;
  }
  if(typeof window.open==='function')window.open(url,'_blank','noopener');
}
function focusResultHeading(){focusNode(el('go-control'))}
...
function renderGauge(){
  ...
  var phase=gaugePhase(),active=['ping','download','upload'].indexOf(phase)>=0;
  ...
  if(active)buildGaugeLabels(phase);setCurrentMetric(active?phase:null);
  ...
  var graphScale=state.traces.download.concat(state.traces.upload).reduce(function(max,value){return Math.max(max,Number(value)||0)},0);
  el('download-trace').setAttribute('d',SpeedtestGauge.tracePath(state.traces.download,graphScale));
  el('upload-trace').setAttribute('d',SpeedtestGauge.tracePath(state.traces.upload,graphScale));
  ...
  var metricsHidden=state.status==='idle'||state.status==='preparing';el('primary-metrics').hidden=metricsHidden;document.querySelector('.latency-strip').hidden=metricsHidden;el('live-graph').hidden=metricsHidden;
...
function render(){setHidden(el('test-stage'),state.view!=='home');var modeLocked=!initialized||state.status==='preparing'||state.status==='running'||state.status==='cancelling';document.querySelectorAll('[data-mode]').forEach(function(b){var selected=b.getAttribute('data-mode')===state.mode;b.classList.toggle('active',selected);b.setAttribute('aria-pressed',selected?'true':'false');b.disabled=modeLocked});document.querySelectorAll('[data-view]').forEach(function(b){if(b.getAttribute('data-view')===state.view)b.setAttribute('aria-current','page');else b.removeAttribute('aria-current')});el('go-control').disabled=!initialized;el('server-picker').disabled=!initialized;text(el('route-label'),state.mode==='device-router'?'Device → Router':state.mode==='both'?'Device → Router + Router → Internet':'Router → Internet');text(el('status'),state.transitionMessage|| (state.status==='preparing'?'Preparing…':state.status==='running'?'Testing…':state.status==='cancelling'?'Cancelling…':state.status==='error'?'Test failed':state.status==='cancelled'?'Test cancelled':state.status==='done'?'Test complete':'Ready to test your connection'));var errorMessage=el('error-message'),retry=el('retry-test');if(errorMessage){errorMessage.hidden=state.status!=='error';text(errorMessage,state.status==='error'?failureMessage()+'. Last valid measurements are retained.':'')}if(retry)retry.hidden=state.status!=='error';text(el('isp-badge'),state.isp||'ISP');text(el('network-badge'),state.connection||'Connection');text(el('vpn-callout'),state.network&&state.network.note||'');var previewServer=state.server.id?state.server:state.autoServer||state.server;text(el('server-name'),previewServer.name||'Auto');text(el('server-detail'),[previewServer.sponsor,previewServer.location||previewServer.city].filter(Boolean).join(' · ')||'Select a server');renderGauge();renderResults();SpeedtestViews.render(el('view'),state.view,state,{deleteHistory:function(id){return checked('delete_history',{id:id}).then(function(){return loadHistory()})},clearHistory:function(){return checked('clear_history',{}).then(function(){return loadHistory()})},saveSettings:saveSettings,openResult:function(row){state.selectedHistory=row;state.view='result';render()},goHome:function(){state.view='home';render()}})}
