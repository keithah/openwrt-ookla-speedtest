...
function cancelTest(){var jobId=state.activeJob;if(state.cancelRequested)return cancelPromise||Promise.resolve({ok:false,pending:!jobId});if(state.status!=='running')return Promise.resolve({ok:false});state.cancelRequested=true;state.status='cancelling';clearPollWake(runGeneration);render();notify();if(!jobId)return Promise.resolve({ok:true,pending:true});return requestCancel(runGeneration,jobId)}
