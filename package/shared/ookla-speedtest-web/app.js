...
function startRun(){var staleLocal=state.status==='running'&&state.localRunId?state.localRunId:null;clearPollWake();cancelPromise=null;cancelPromiseToken=null;localCancelPromise=null;localCancelPromiseToken=null;localCancelOutcome=null;localCancelOutcomeToken=null;var next=++runGeneration;if(staleLocal)Promise.resolve().then(function(){return checked('cancel_local',{run_id:staleLocal})}).catch(function(){});return next}
...
function announcePhase(phase){if(phase===lastAnnouncedPhase)return;lastAnnouncedPhase=phase;var message=phase==='complete'?'Test complete':phase==='cancelled'?'Test cancelled':phase==='error'?'Test failed':phase==='idle'?'Ready':phase.charAt(0).toUpperCase()+phase.slice(1)+' phase';text(el('phase-announcer'),message)}
