...
function traceValues(rows){if(rows==null)return null;if(!Array.isArray(rows))throw liveError('malformed_live_status');return rows.map(function(row){if(!row||typeof row!=='object')throw liveError('malformed_live_status');return liveNumber(row.value,100000)}).slice(-120)}
...
function pollLive(jobId,runToken){
  ...
  return checked('live_status',{job_id:jobId}).then(function(payload){
    ...
    if(state.cancelRequested)throw liveError('cancelled');
    state.pollFailures=0;
    if(!applyLiveStatus(payload,jobId))return new Promise(function(resolve){setTimeout(resolve,500)}).then(function(){return pollLive(jobId,runToken)});
    ...
    return new Promise(function(resolve){setTimeout(resolve,500)}).then(function(){return pollLive(jobId,runToken)});
  },function(error){
    if(!ownsRun(runToken,jobId))throw liveError('stale_live_job');
    if(state.cancelRequested)throw liveError('cancelled');
    ...
    return new Promise(function(resolve){setTimeout(resolve,wait)}).then(function(){return pollLive(jobId,runToken)});
  }).catch(function(error){if(error.code==='malformed_live_status'&&ownsRun(runToken,jobId))setLiveError(error,jobId);throw error});
}
function internetTest(runToken){if(runToken==null)runToken=++runGeneration;var params={};if(state.server.id)params.server_id=state.server.id;state.cancelRequested=false;state.pollFailures=0;return checked('start_live',params).then(function(started){if(!ownsRun(runToken))throw liveError('stale_live_job');if(!started.job_id)throw liveError('malformed_live_status');state.activeJob=started.job_id;state.status='running';state.phase='ping';render();notify();return pollLive(started.job_id,runToken)})}
