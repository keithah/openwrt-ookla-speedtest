...
function cleanupLocalRecord(response,runError){var id=response&&response.item&&response.item.id;if(id==null)return Promise.reject(liveError('local_cleanup_failed','Cancelled local result could not be removed'));return checked('delete_history',{id:id}).then(function(){throw runError},function(){throw liveError('local_cleanup_failed','Cancelled local result could not be removed')})}
function localTest(runToken){if(runToken==null)runToken=startRun();var size=32768,payload=new Array(size+1).join('0'),options=localConfig(),ping;state.status='running';state.phase='ping';render();notify();return localPing(runToken).then(function(value){ping=value;return localTransfer('local_download',{bytes:size},'download',runToken,options)}).then(function(down){return localTransfer('local_upload',{data:payload},'upload',runToken,options).then(function(up){requireLocalRun(runToken);return checked('record_local',{download_mbps:String(down),upload_mbps:String(up),ping_ms:String(ping)}).then(function(response){try{requireLocalRun(runToken)}catch(error){return cleanupLocalRecord(response,error)}return{kind:'device-router',download_mbps:down,upload_mbps:up,ping_ms:ping}},function(error){requireLocalRun(runToken);throw error})})})}
...
function startRun(){clearPollWake();cancelPromise=null;cancelPromiseToken=null;return++runGeneration}
...
function cancelTest(){var jobId=state.activeJob;if(state.cancelRequested)return cancelPromise||Promise.resolve({ok:false,pending:!jobId});if(state.status!=='running')return Promise.resolve({ok:false});state.cancelRequested=true;state.status='cancelling';clearPollWake(runGeneration);if(!jobId&&state.phase!=='idle'){state.status='cancelled';state.phase='cancelled'}render();notify();if(!jobId)return Promise.resolve({ok:true,pending:true});return requestCancel(runGeneration,jobId)}
...
function resetMeasurement(phase){state.status='running';state.phase=phase;state.progress=0;state.gaugeValue=0;state.gaugeUnit='Mbps';state.gaugeScale=100;state.traces={download:[],upload:[]};state.activeJob=null;state.pollFailures=0;state.download=null;state.upload=null;state.ping=null;state.jitter=null;state.loss=null;render();notify()}
