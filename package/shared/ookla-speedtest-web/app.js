...
function resultCard(title,r,type){var card=document.createElement('article');card.className='result-card '+type;var h=document.createElement('h2');h.textContent=title;card.appendChild(h);var rows=[['Download',r.download_mbps,'Mbps'],['Upload',r.upload_mbps,'Mbps'],['Ping',r.ping_ms,'ms']];if(type==='internet')rows=rows.concat([['Jitter',r.jitter_ms,'ms'],['Loss',r.loss_percent,'%']]);rows.forEach(function(row){var p=document.createElement('p');p.textContent=row[0]+' '+(row[1]==null?'—':row[1])+' '+row[2];card.appendChild(p)});var details=[];if(r.server)details.push('Server: '+[r.server.name,r.server.sponsor,r.server.location||r.server.city].filter(Boolean).join(' · '));if(r.isp)details.push('ISP: '+r.isp);if(r.interface)details.push('Interface: '+(r.interface.name||r.interface));if(r.network_context){var context=r.network_context.note||r.network_context.connection||'';if(r.network_context.vpn===true||r.network_context.is_vpn===true)context=(context?context+' · ':'')+'VPN';if(context)details.push('Connection: '+context)}details.forEach(function(value){var small=document.createElement('small');small.textContent=value;card.appendChild(small)});return card}
function renderResults(){var box=el('results');while(box&&box.firstChild)box.removeChild(box.firstChild);if(!box)return;if((state.mode==='device-router'||state.mode==='both')&&state.results.local)box.appendChild(resultCard('Device → Router',state.results.local,'local'));if((state.mode==='router-internet'||state.mode==='both')&&state.results.internet)box.appendChild(resultCard('Router → Internet',state.results.internet,'internet'))}
...
function failedPathLabel(){return state.errorPath==='local'?'Device → Router':'Router → Internet'}
function failureMessage(){return failedPathLabel()+' '+meaningfulPhase(state.failedPhase)+' failed'+(state.errorCode?' ('+state.errorCode+')':'')}
...
function renderGauge(){
  ...
  gauge.setAttribute('data-phase',phase);gauge.setAttribute('data-status',state.status);gauge.setAttribute('aria-busy',state.status==='running'||state.status==='cancelling'?'true':'false');gauge.style.setProperty('--gauge-progress',String(progress));
...
function historyMbps(r,key){if(r[key+'_mbps']!=null)return metric(r[key+'_mbps'])+' Mbps';var bandwidth=r[key]&&r[key].bandwidth;if(bandwidth==null)return '—';return metric(r.kind==='router-internet'?bandwidth*8/1000000:bandwidth)+' Mbps'}
function historyOutcome(r){if(r.outcome==='cancelled')return'Cancelled';if(r.outcome&&r.outcome!=='success'){var code=r.error_code||r.error&&r.error.code;return'Failed'+(code?' ('+code+')':'')}return'Success'}
function outcomeClass(outcome){return outcome==='error'||outcome==='failed'?'failed':outcome||'success'}
function renderHistory(v){var h=document.createElement('h2');h.textContent='History';v.appendChild(h);var region=document.createElement('div');region.className='history-scroll';region.setAttribute('role','region');region.setAttribute('aria-label','Speedtest history table');region.setAttribute('tabindex','0');var table=document.createElement('table');var head=document.createElement('tr');['Path','Date','Outcome','Download','Upload','Ping','Actions'].forEach(function(x){var th=document.createElement('th');th.textContent=x;head.appendChild(th)});table.appendChild(head);state.history.forEach(function(r){var tr=document.createElement('tr'),outcome=historyOutcome(r),vals=[r.kind==='device-router'?'Device → Router':'Router → Internet',r.date||new Date((r.timestamp||0)*1000).toLocaleString(),outcome,historyMbps(r,'download'),historyMbps(r,'upload'),r.ping_ms!=null?r.ping_ms+' ms':r.latency!=null?r.latency+' ms':'—'];vals.forEach(function(x,index){var td=document.createElement('td');td.textContent=x;if(index===2)td.className='outcome '+outcomeClass(r.outcome);tr.appendChild(td)});var td=document.createElement('td'),b=document.createElement('button');b.textContent='Delete';b.onclick=function(){checked('delete_history',{id:r.id}).then(loadHistory).catch(function(){})};td.appendChild(b);tr.appendChild(td);table.appendChild(tr)});region.appendChild(table);v.appendChild(region)}
function renderAnalytics(v){var h=document.createElement('h2');h.textContent='Analytics';v.appendChild(h);['router-internet','device-router'].forEach(function(kind){var rows=state.history.filter(function(r){return r.kind===kind&&r.outcome==='success'});var p=document.createElement('p');p.textContent=(kind==='device-router'?'Device → Router':'Router → Internet')+': '+rows.length+' recorded test'+(rows.length===1?'':'s');v.appendChild(p)})}
function render(){var modeLocked=state.status==='running'||state.status==='cancelling';document.querySelectorAll('[data-mode]').forEach(function(b){var selected=b.getAttribute('data-mode')===state.mode;b.classList.toggle('active',selected);b.setAttribute('aria-pressed',selected?'true':'false');b.disabled=modeLocked});text(el('route-label'),state.mode==='device-router'?'Device → Router':state.mode==='both'?'Device → Router + Router → Internet':'Router → Internet');text(el('scope-note'),state.mode==='device-router'?'Tests this device’s Wi‑Fi or Ethernet path to the router—not the internet.':state.mode==='both'?'Runs both paths from one action and reports them separately.':'Tests the router’s connection to the internet—not this device.');text(el('status'),state.status==='running'?'Testing…':state.status==='cancelling'?'Cancelling…':state.status==='error'?'Test failed':state.status==='cancelled'?'Test cancelled':state.status==='done'?'Test complete':'Ready to test your connection');var errorMessage=el('error-message'),retry=el('retry-test');if(errorMessage){errorMessage.hidden=state.status!=='error';text(errorMessage,state.status==='error'?failureMessage()+'. Last valid measurements are retained.':'')}if(retry)retry.hidden=state.status!=='error';text(el('isp-badge'),state.isp||'ISP');text(el('network-badge'),state.connection||'Connection');text(el('vpn-callout'),state.network&&state.network.note||'');text(el('server-name'),state.server.name||'Auto');text(el('server-detail'),[state.server.sponsor,state.server.location||state.server.city].filter(Boolean).join(' · ')||'Select a server');renderGauge();renderResults();var v=el('view');while(v&&v.firstChild)v.removeChild(v.firstChild);if(!v||state.view==='home')return;if(state.view==='history')renderHistory(v);else if(state.view==='analytics')renderAnalytics(v);else if(state.view==='settings')v.appendChild(document.createTextNode('History and acceptance are stored locally on this router.'));else if(state.view==='about')v.appendChild(document.createTextNode('Unofficial OpenWrt frontend. Device → Router measures this browser’s authenticated RPC path to the router. Router → Internet runs the separate Ookla CLI on the router. Both results are stored and displayed separately.'))}
function resetMeasurement(phase){clearNumericAnnouncement();state.status='running';state.phase=phase;state.failedPhase=null;state.progress=0;state.gaugeValue=0;state.gaugeUnit='Mbps';state.gaugeScale=100;state.traces={download:[],upload:[]};state.activeJob=null;state.localRunId=null;state.pollFailures=0;state.download=null;state.upload=null;state.ping=null;state.jitter=null;state.loss=null;render();notify()}
function executeMode(mode,runToken){if(runToken==null)runToken=startRun();state.failedMode=mode;state.cancelRequested=false;state.errorPath=null;state.errorCode=null;state.results={internet:null,local:null};resetMeasurement(mode==='router-internet'?'idle':'ping');var currentPath=mode==='router-internet'?'internet':'local',work;if(mode==='device-router'){work=localTest(runToken).then(function(local){if(ownsRun(runToken))state.results.local=local;return local})}else if(mode==='router-internet'){work=internetTest(runToken).then(function(internet){if(ownsRun(runToken))state.results.internet=internet;return internet})}else{work=localTest(runToken).then(function(local){if(!ownsRun(runToken))throw liveError('stale_local_test');state.results.local=local;currentPath='internet';resetMeasurement('idle');return internetTest(runToken)}).then(function(internet){if(ownsRun(runToken))state.results.internet=internet;return internet})}return work.then(function(){if(!ownsRun(runToken))return;var summary=state.results.internet||state.results.local;if(summary){state.download=summary.download_mbps;state.upload=summary.upload_mbps;state.ping=summary.ping_ms;state.jitter=summary.jitter_ms;state.loss=summary.loss_percent;state.gaugeValue=summary.download_mbps||0;state.gaugeScale=SpeedtestGauge.scaleFor(state.gaugeValue,state.gaugeScale);state.progress=100}if(state.results.internet){var internet=state.results.internet;state.isp=internet.isp||state.isp;state.connection=internet.interface&&internet.interface.name||state.connection;state.network=internet.network_context;if(internet.server)state.server=internet.server}state.status='done';state.phase='complete';state.failedPhase=null;state.failedMode=null;clearPollWake(runToken);render();notify();return loadHistory(runToken).catch(function(){})}).catch(function(err){if(!ownsRun(runToken))return;if(err.code==='terms_required'){state.failedPhase=null;state.status='idle';state.phase='idle';state.pendingMode=mode;el('terms-dialog').showModal()}else if(err.code==='cancelled'){state.failedPhase=null;state.status='cancelled';state.phase='cancelled'}else{state.failedPhase=meaningfulPhase(state.failedPhase,meaningfulPhase(state.phase,currentPath==='internet'?'starting':'ping'));state.errorPath=currentPath;state.errorCode=err.code||err.message||'unknown_error';state.status='error';state.phase='error'}clearPollWake(runToken);render();notify()})}
function runMode(mode){var runToken=startRun();state.mode=mode;state.failedMode=mode;state.failedPhase=null;state.status='running';state.phase='starting';render();notify();if(mode==='device-router')return executeMode(mode,runToken);return checked('settings',{}).then(function(settings){if(!ownsRun(runToken))return;if(!settings.terms_accepted){state.status='idle';state.phase='idle';state.pendingMode=mode;render();notify();el('terms-dialog').showModal();return}return executeMode(mode,runToken)}).catch(function(err){if(!ownsRun(runToken))return;state.errorPath='internet';state.errorCode=err.code||err.message||'settings_failed';state.failedPhase='starting';state.status='error';state.phase='error';clearPollWake(runToken);render();notify();throw err})}
...
document.addEventListener('DOMContentLoaded',function(){document.querySelectorAll('[data-mode]').forEach(function(b){b.addEventListener('click',function(){if(state.status==='running'||state.status==='cancelling')return;state.mode=b.getAttribute('data-mode');render()})});document.querySelectorAll('[data-view]').forEach(function(b){b.addEventListener('click',function(){state.view=b.getAttribute('data-view');render()})});el('go-control').addEventListener('click',function(){return runMode(state.mode).catch(function(){})});el('retry-test').addEventListener('click',function(){return runMode(state.failedMode||state.mode).catch(function(){})});el('cancel-test').addEventListener('click',function(){return Promise.resolve().then(cancelTest).catch(function(){})});el('accept-terms').addEventListener('click',function(){var mode=state.pendingMode||state.mode;return checked('accept_terms',{}).then(function(){state.pendingMode=null;return runMode(mode)}).catch(function(err){state.failedMode=mode;state.errorPath=mode==='device-router'?'local':'internet';state.errorCode=err.code||err.message||'terms_acceptance_failed';state.failedPhase='starting';state.status='error';state.phase='error';render();notify()})});el('server-picker').addEventListener('click',function(){el('server-panel').hidden=false;return checked('servers',{}).then(function(x){state.servers=x.servers||x||[]}).catch(function(){})});el('server-search').addEventListener('input',function(){var q=this.value.toLowerCase(),box=el('server-results');while(box.firstChild)box.removeChild(box.firstChild);state.servers.filter(function(s){return(s.name||'').toLowerCase().indexOf(q)>=0}).forEach(function(s){var b=document.createElement('button');b.textContent=(s.name||'Server')+' '+(s.location||'');b.onclick=function(){state.server=s;el('server-panel').hidden=true;render()};box.appendChild(b)})});loadHistory().catch(function(){render()})});
