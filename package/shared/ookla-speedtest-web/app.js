...
document.addEventListener('DOMContentLoaded',function(){createNeedleAnimator();el('go-control').disabled=true;el('server-picker').disabled=true;document.querySelectorAll('[data-mode]').forEach(function(b){b.disabled=true;b.addEventListener('click',function(){if(!initialized||state.status==='running'||state.status==='cancelling')return;state.mode=b.getAttribute('data-mode');render()})});document.querySelectorAll('[data-view]').forEach(function(b){b.addEventListener('click',function(){if(!initialized)return;state.view=b.getAttribute('data-view');render()})});el('go-control').addEventListener('click',function(){if(!initialized)return;return runMode(state.mode).catch(function(){})});el('retry-test').addEventListener('click',function(){if(!initialized)return;return runMode(state.failedMode||state.mode).catch(function(){})});el('cancel-test').addEventListener('click',function(){if(!initialized)return;return Promise.resolve().then(cancelTest).catch(function(){})});el('accept-terms').addEventListener('click',function(){if(!initialized)return;var mode=state.pendingMode||state.mode;return checked('accept_terms',{}).then(function(){state.pendingMode=null;return runMode(mode)}).catch(function(err){state.failedMode=mode;state.errorPath=mode==='device-router'?'local':'internet';state.errorCode=err.code||err.message||'terms_acceptance_failed';state.failedPhase='starting';state.status='error';state.phase='error';render();notify()})});el('server-picker').addEventListener('click',function(){if(!initialized)return;el('server-panel').hidden=false;return checked('servers',{}).then(function(x){state.servers=x.servers||x||[];renderServerChoices()}).catch(function(){})});el('server-search').addEventListener('input',function(){if(initialized)renderServerChoices()});initialization=checked('settings',{}).then(function(settings){applySettings(settings)},function(){}).then(function(){initialized=true;render();notify();return loadHistory()}).catch(function(){render()})});
