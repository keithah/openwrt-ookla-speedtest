...
function releaseFailedLocalRun(runId,error){if(!runId||error.code==='cancelled')return Promise.reject(error);return checked('cancel_local',{run_id:runId}).then(function(){throw error},function(cleanupError){if(cleanupError.code==='too_late'&&error.localCandidate)return error.localCandidate;throw error})}
function localTest(runToken){if(runToken==null)runToken=startRun();var size=32768,payload=new Array(size+1).join('0'),options=localConfig(),ping,runId;state.status='running';state.phase='ping';render();notify();return checked('begin_local',{}).then(function(response){runId=localRunId(response.run_id);if(!ownsRun(runToken))return checked('cancel_local',{run_id:runId}).then(function(){throw liveError('stale_local_test')},function(){throw liveError('stale_local_test')});state.localRunId=runId;if(state.cancelRequested)return requestLocalCancel(runToken,runId).then(function(outcome){if(outcome&&outcome.state==='cancelled')throw liveError('cancelled');return runId});return runId}).then(function(){return localPing(runToken,runId)}).then(function(value){ping=value;return localTransfer('local_download',{run_id:runId,bytes:size},'download',runToken,options)}).then(function(down){return localTransfer('local_upload',{run_id:runId,data:payload},'upload',runToken,options).then(function(up){var candidate={kind:'device-router',download_mbps:down,upload_mbps:up,ping_ms:ping};requireLocalRun(runToken);return checked('record_local',{run_id:runId,download_mbps:String(down),upload_mbps:String(up),ping_ms:String(ping)}).then(function(response){if(response.state!=='committed')throw liveError('malformed_local_run');if(!ownsRun(runToken))throw liveError('stale_local_test');state.cancelRequested=false;state.status='running';return candidate},function(error){requireLocalRun(runToken);error.localCandidate=candidate;throw error})})}).catch(function(error){return releaseFailedLocalRun(runId,error)})}
