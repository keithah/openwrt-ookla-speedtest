...
function startRun(){var staleLocal=state.status==='running'&&state.localRunId?state.localRunId:null;clearPollWake();state.failedPhase=null;cancelPromise=null;cancelPromiseToken=null;localCancelPromise=null;localCancelPromiseToken=null;localCancelOutcome=null;localCancelOutcomeToken=null;var next=++runGeneration;if(staleLocal)Promise.resolve().then(function(){return checked('cancel_local',{run_id:staleLocal})}).catch(function(){});return next}
...
function renderGauge(){
  ...
  announcePhase(state.phase);if(state.status==='running')announceGauge(phase+' '+metric(state.gaugeValue)+' '+(state.gaugeUnit||'Mbps'));
}
...
function renderHistory(v){var h=document.createElement('h2');h.textContent='History';v.appendChild(h);var table=document.createElement('table');var head=document.createElement('tr');['Path','Date','Outcome','Download','Upload','Ping','Actions'].forEach(function(x){var th=document.createElement('th');th.textContent=x;head.appendChild(th)});table.appendChild(head);state.history.forEach(function(r){var tr=document.createElement('tr'),outcome=historyOutcome(r),vals=[r.kind==='device-router'?'Device → Router':'Router → Internet',r.date||new Date((r.timestamp||0)*1000).toLocaleString(),outcome,historyMbps(r,'download'),historyMbps(r,'upload'),r.ping_ms!=null?r.ping_ms+' ms':r.latency!=null?r.latency+' ms':'—'];vals.forEach(function(x,index){var td=document.createElement('td');td.textContent=x;if(index===2)td.className='outcome '+(r.outcome||'success');tr.appendChild(td)});var td=document.createElement('td'),b=document.createElement('button');b.textContent='Delete';b.onclick=function(){checked('delete_history',{id:r.id}).then(loadHistory).catch(function(){})};td.appendChild(b);tr.appendChild(td);table.appendChild(tr)});v.appendChild(table)}
