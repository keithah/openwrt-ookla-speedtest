...
function morphGaugePhase(phase){
  ...
  if(previousActive&&previous!==phase){
    ...
    if(window.matchMedia('(prefers-reduced-motion: reduce)').matches){phaseResetFrame=null;if(active)retargetGauge(phase);pendingGaugeTarget=null;return}
    waitForGaugeReset();return
  }
  if(phaseResetFrame!=null&&active){pendingGaugeTarget=phase;if(window.matchMedia('(prefers-reduced-motion: reduce)').matches){window.cancelAnimationFrame(phaseResetFrame);phaseResetFrame=null;retargetGauge(phase);pendingGaugeTarget=null}}
}
